...
namespace AridiaEditor.Gizmo
{
  public class GizmoComponent
  {
    ...
    // -- Models -- //
    private readonly Model _translationModel;
    private readonly Model _rotationModel;
    private readonly Model _scaleModel;
    ...
    // -- Selection -- //
    public List<Actor> Selection = new List<Actor>();
    private IEnumerable<Actor> _selectionPool = null;
    ...
    public GizmoComponent(GraphicsDevice graphics, SpriteBatch spriteBatch)
      : this(graphics, spriteBatch, Matrix.Identity) {}

    public GizmoComponent(GraphicsDevice graphics, SpriteBatch spriteBatch, Matrix world)
    {
      SceneWorld = world;
      _graphics = graphics;
      ...
      _lineEffect = new BasicEffect(graphics)
                      {VertexColorEnabled = true, AmbientLightColor = Vector3.One, EmissiveColor = Vector3.One};
      _quadEffect = new BasicEffect(graphics)
                      {World = Matrix.Identity, DiffuseColor = _highlightColor.ToVector3(), Alpha = 0.5f};
      _quadEffect.EnableDefaultLighting();

      
      _translationModel = ResourceManager.Instance.GetResource<Model>("gizmo_translate");
      _rotationModel = ResourceManager.Instance.GetResource<Model>("gizmo_rotate");
      _scaleModel = ResourceManager.Instance.GetResource<Model>("gizmo_scale");
      _font = ResourceManager.Instance.GetResource<SpriteFont>("gizmoFont");
    ...
    public void SetSelectionPool(IEnumerable<Actor> selectables)
    {
      _selectionPool = selectables;
    }

    public IEnumerable<Actor> GetSelectionPool()
    {
    ...
    public void Draw()
    {
      ...
      {
        case GizmoMode.Translate:
          activeModel = _translationModel;
          break;
        case GizmoMode.Rotate:
          activeModel = _rotationModel;
          break;
        default:
          activeModel = _scaleModel;
          break;
      }
    ...
    private void CreateSelectionBox()
    {
      ...
      {
        BoundingBox boundingBox = transformable.GetBoundingBox();
    ...
    private void OnScaleEvent(Actor transformable, Vector3 delta)
    {
      if (ScaleEvent != null)
  ...
  public delegate void TransformationEventHandler(Actor transformable, TransformationEventArgs e);
